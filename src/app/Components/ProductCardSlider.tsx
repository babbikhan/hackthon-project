...
const ProductCardSlider: React.FC<ProductCardProps> = ({ item }) => {
  return (
    <Link href={"/product"}>
      <div className=" flex flex-col items-center justify-center group transition-all duration-200 hover:scale-105 my-2">
        {/* image  */}
        <div className="relative">
          <img
            src={item?.image}
            className="md:w-[230px]  md:h-[230px] h-[300px] rounded-[8px] "
          />
          {/* {item?.flag == "new" ? (
